...
void func_to_recursive_line_39_to_45_0(unsigned int (*array)[10000], unsigned int *index, unsigned int *tmp)
{
  if (1)
  {
    ...
    static myBool pStored0 = myFalse;
      myBool flag = __VERIFIER_nondet_myBool(); // BOUNTY
      static unsigned int oldarray[10000],  *oldindex, oldindexval, *oldtmp, oldtmpval; // BOUNTY
      if (pStored0) __CPROVER_assert(! (__CPROVER_array_equal(oldarray, array)  && oldindex == index && oldindexval == *index && oldtmp == tmp && *oldtmp == oldtmpval), "recurrent state found"); // BOUNTY
      if (flag) {  __CPROVER_array_copy(oldarray, array); oldindex = index;  oldindexval = *index; oldtmp = tmp; oldtmpval = *tmp;  pStored0 = myTrue; } // BOUNTY
