...
void safe_search (node_t* head, int i)
{
  ...
  while (curr != ((void *)0) && curr->val != i) {
      ...
      static int *oldcurr, oldcurrv, *oldnext; // BOUNTY
      if (pStored0) __CPROVER_assert(! (oldcurr == curr && oldcurrv == curr->val && oldnext == curr->next), "recurrent state found"); // BOUNTY
      if (flag) {  oldcurr = curr;  oldcurrv = curr->val; oldnext = curr->next; pStored0 = myTrue; } // BOUNTY
    curr = curr->next;
  }
}
int main ()
