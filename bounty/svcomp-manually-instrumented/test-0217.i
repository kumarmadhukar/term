...
void reach_error() { ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "test-0217.c", 3, __extension__ __PRETTY_FUNCTION__); })); }
...
int main()
{
    ...
    while (__VERIFIER_nondet_int()) {
      ...
      static list_t oldlist; // BOUNTY
      if (pStored0) __CPROVER_assert(! (__CPROVER_array_equal(oldlist, list)) , "recurrent state found"); // BOUNTY
      if (flag) {  __CPROVER_array_copy(oldlist, list); pStored0 = myTrue; } // BOUNTY
        while (__VERIFIER_nondet_int())
