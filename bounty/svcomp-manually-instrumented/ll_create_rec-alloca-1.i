...
node_t* new_ll(unsigned int n)
{
  ...
  static int *oldhead, oldval, *oldnext; // BOUNTY
  if (pStored0) __CPROVER_assert(! (oldhead == head && oldval == head->val && oldnext == head->next), "recurrent state found"); // BOUNTY
  if (flag) {  oldhead = head;  oldval = head->val; oldnext = head->next; pStored0 = myTrue; } // BOUNTY
