...
void traverse (node_t* head)
{
  ...
  while (curr != ((void *)0)) {
      ...
      static int *oldcurr, oldcurrv; // BOUNTY
      if (pStored0) __CPROVER_assert(! (oldcurr == curr && oldcurrv == curr->val), "recurrent state found"); // BOUNTY
      if (flag) {  oldcurr = curr;  oldcurrv = curr->val; pStored0 = myTrue; } // BOUNTY
    curr = curr->next ;
  }
}
int main ()
